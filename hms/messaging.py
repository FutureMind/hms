...
from hms import _messages, _app
import hms
...
def send_message(message, validate_only=False, app_id=None):
    ...
    """
    try:
        response = hms.get_app(app_id).send(message, validate_only)
...
def subscribe_topic(topic, token_list, app_id=None):
    ...
    """
    try:
        response = hms.get_app(app_id).subscribe_topic(topic, token_list)
...
def unsubscribe_topic(topic, token_list, app_id=None):
    ...
    """
    try:
        response = hms.get_app(app_id).unsubscribe_topic(topic, token_list)
        return TopicSubscribeResponse(response)
    except Exception as e:
        raise ApiCallError(repr(e))
...
def list_topics(token, app_id=None):
    ...
    """
    try:
        response = hms.get_app(app_id).query_subscribe_list(token)
